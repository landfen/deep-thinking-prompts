...
> 两个长流程深度对话 prompt：隐藏天赋往过去看（"我身上到底有什么"），人生设计术往未来看（"我接下来可以往哪去"）。单次使用通常需要半小时以上，回答越真实具体，产出越有价值。

---

## 1. 挖掘隐藏天赋

**适用场景**：想知道自己的天赋优势；觉得自己"没啥天赋"；想找到被压抑/误解的能力。产出约一万字的《个人天赋使用说明书》，分章交付（先提纲后逐章）。对话规则严格：每次只问一个问题，全程最多 10 个主问题，覆盖四条主线。
...
## 输出
当信息足够丰富后，输出一份一万字左右的《个人天赋使用说明书》。按以下六章顺序分章交付：先输出六章标题与每章要点组成的提纲供用户确认，确认后逐章完成，每章1500～2000字；章节顺序可根据用户的回答微调，但六章必须齐全：
...
**适用场景**：基于斯坦福人生设计方法（Designing Your Life），规划未来的职业/人生方向。产出 8000～12000 字的《个人人生设计蓝图》，分章交付（先提纲后逐章），含三个奥德赛计划与原型行动清单。主问题控制在 6～9 个，分四个阶段推进。
...
## 输出
当素材足够丰富后，输出一份8000到12000字的《个人人生设计蓝图》，分章交付：先输出提纲供用户确认，再逐章完成，每章1500～2000字，自然覆盖：
...
- 这两个 prompt 是**长流程交互**：严格执行"每次只问一个问题"的节奏，禁止一次性抛出问卷。
- 用户情绪低落时注意模板内的保护性条款（如反向推演步骤在用户状态不稳定时应跳过）。
- 最终说明书/蓝图为长文交付物：分章交付（先提纲供确认，再逐章完成），逐章完整输出，不可摘要化；在支持文件写入的环境中将全文落盘保存。
